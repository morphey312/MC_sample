...
import ReportTable from "./marketing/ReportTable.vue";
    ...
    methods: {
        ...
        getSpecializations(filters = []) {
            ...
            return specialization.fetchList().then(response => {
                let specializations = [];
                const prioritySpecializationsShortNames = ['ГИН', 'УЗИ', 'ПРО', 'УРО', 'ДЕРМ', 'ГАСТРО'];

                const prioritySpecializations = response.filter(item => {
                    return prioritySpecializationsShortNames.includes(item.short_name);
                })

                const otherSpecializations = response.filter(item => {
                    return !prioritySpecializationsShortNames.includes(item.short_name);
                })

                specializations = [...prioritySpecializations, ...otherSpecializations];
        ...
        getInformationSources(filters = []) {
            ...
            return informationSources.fetchList({is_active: [0, 1]}, null, 5000).then(response => {

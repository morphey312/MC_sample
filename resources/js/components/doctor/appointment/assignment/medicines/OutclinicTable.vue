...
export default {
    ...
    data() {
        return {
            ...
            filteredResults: this.medicines,
            initialSortOrder: [
    ...
    watch: {
        ['medicines']() {
            this.filteredResults = this.medicines;
            this.refresh();
        },
    },
    methods: {
        ...
        updateList(updates) {
            ...
            this.filteredResults = this.filterResults(this.medicines,  _.onlyFilled(this.filter));
            this.refresh();
        },
        sortResults(sort) {
            if (!sort[0]) {
        ...
        remove(row, index) {
            let model = new OutclinicMedicine({...row, is_deleted: true});
            model.save().then(() => {
                this.$emit('medicine-deleted');
            });
        },
    },
}

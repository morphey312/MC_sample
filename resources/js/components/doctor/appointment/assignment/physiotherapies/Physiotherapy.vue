...
export default {
    props: {
        ...
        title: {
            type: [Boolean, String],
            default: __('Назначена физиотерапия'),
        },
        readonly: {
            type: Boolean,
            default: false,
    ...
    computed: {
        total(){
            return this.physiotherapies.reduce((total, row) => {
    ...
    methods: {
        remove(index) {
            this.$emit('remove-physiotherapy', index);
        }
    }
}
</script>

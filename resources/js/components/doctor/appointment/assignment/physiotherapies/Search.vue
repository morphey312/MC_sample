...
export default {
    ...
    data() {
        return {
            selectedRows: this.getPhysiotherapies(),
            model: {},
    ...
    methods: {
        ...
        getPhysiotherapies() {
            return this.physiotherapies.map(service => new AssignedService(service._attributes));
        },
        getFeaturedFilter() {
        ...
        getDefaultFilters() {
            return this.getFilterValues({
                specialization_group: CONSTANTS.SPECIALIZATION.SERVICE_GROUPS.PHYSIOTHERAPY,
            });
        },
        removeFromSelected({row, index}) {
            ...
            if (model !== undefined && !model.isNew()) {
                model.delete().then((response) => {
                    ...
                    return this.$info(__('Физиотерапия успешно удалена'));
                }).catch((error) => {
                    return this.$error(__('Не удалось удалить назначенную физиотерапию'));
                });
            } else {
                this.selectedRows.splice(index, 1);
            }

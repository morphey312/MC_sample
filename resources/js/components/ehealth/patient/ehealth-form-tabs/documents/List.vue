...
export default {
        ...
        return {
            repository: new ProxyRepository(() => {
                return Promise.resolve({
                    rows: this.patient.patient_documents,
                });
            }),
            fields: [
                {
    ...
    methods: {
        ...
        onCreated(model) {
            this.patient.patient_documents.push(model);
        },
        performDelete(model) {
            let index = this.patient.patient_documents.indexOf(model);
            if (index !== -1) {
                this.patient.patient_documents.splice(index, 1);
            }
            return Promise.resolve();
        },
    }

...
export default {
        ...
        return {
            repository: new ProxyRepository(() => {
                return Promise.resolve({
                    rows: this.patient.patient_relationship_documents
                });
            }),
            fields: [
                {
                    ...
                    width: '30%',
                    formatter: (value) => {
                        return this.$handbook.getOption('ehealth_document_relationship_type', value);
                    },
                },
                {
                    name: 'number',
    ...
    methods: {
        ...
        onCreated(model) {
            this.patient.patient_relationship_documents.push(model);
        },
        performDelete(model) {
            let index = this.patient.patient_relationship_documents.indexOf(model);
            if (index !== -1) {
                this.patient.patient_relationship_documents.splice(index, 1);
            }
            return Promise.resolve();
        },
    }

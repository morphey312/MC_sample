...
/**
 * Class SendPhoneNotReachedContacts
 * @package App\V1\Jobs\Esputnik
 */
class SendPhoneNotReachedContacts implements ShouldQueue
{
     ...
     * @return void
     */
    public function handle() // TODO: Оптимизация запроса
    {
        $clinicsFromTemplate = $this->template->settings_clinics()->get()->pluck('clinic_id')->toArray();

        if ($ids = $this->getAffectedPatientIds()) {
            ...
            $contact
                ...
                ->setOption('groupNames', ['CallNoAnswer']);
            ...
            $patientQuery->chunk(20, function($patients) use ($contact){
                foreach ($patients as $patient) {
                    ...
                    $contact
                        ->addCustomField(166983, $this->date->toDateString());
        ...
        $siteEnquiries = SiteEnquiry::query()
            ...
            ->whereExists(function ($query) {
                $query
                    ...
                    ->where('date', $this->date->toDateString())
                    ->where('call_results.esputnik_no_answer', 1);
            });

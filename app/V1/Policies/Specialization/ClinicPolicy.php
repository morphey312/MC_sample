...
namespace App\V1\Policies\Specialization;
...
class ClinicPolicy extends ClinicSharedPolicy
{
    ...
    public function create(User $user)
    {
        return $this->can($user, self::ACTION_CREATE) || $this->can($user, self::ACTION_UPDATE)
            || $this->can($user, self::ACTION_CREATE_CLINIC) || $this->can($user, self::ACTION_CREATE_CLINIC);
    }
    ...
    public function update(User $user, Model $model)
    {
        return ($this->can($user, self::ACTION_CREATE) || $this->can($user, self::ACTION_UPDATE)
                || $this->can($user, self::ACTION_CREATE_CLINIC) || $this->can($user, self::ACTION_CREATE_CLINIC))
            && $this->isOwnedBy($model->specialization, $user);
    }
